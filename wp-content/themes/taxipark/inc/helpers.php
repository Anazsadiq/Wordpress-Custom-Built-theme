...
function fw_theme_font_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Lato font: on or off', 'taxipark' ) ) {
					...
					<div class="contributor-summary">
						...
						<a class="button contributor-posts-link"
						   href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>">
							<?php printf( _n( '%d Article', '%d Articles', $post_count, 'taxipark' ), $post_count ); ?>
						</a>
					</div>
					<!-- .contributor-summary -->
				</div>
...
if ( ! function_exists( 'fw_theme_paging_nav' ) ) : /**
	...
	function fw_theme_paging_nav( $wp_query = null ) {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( 'prev', 'taxipark' ),
			'next_text' => esc_html__( 'next', 'taxipark' ),
		) );
		...
		<nav class="navigation paging-navigation clearfix" role="navigation">
			<h1 class="screen-reader-text"><?php echo esc_html__( 'Posts navigation', 'taxipark' ); ?></h1>
			<hr />
			<div class="pagination loop-pagination">
				<?php if ( $paged === 1 ) { echo '<a href="#" class="prev page-numbers disabled">' . esc_html__( 'prev', 'taxipark' ) . '</a>';} ?>
				<?php echo wp_kses_post( $links ); ?>
				<?php if ( $paged === $wp_query->max_num_pages ) { echo '<a href="#" class="next page-numbers disabled">' . esc_html__( 'next', 'taxipark' ) . '</a>';} ?>
			</div>
			<!-- .pagination -->
				</nav><!-- .navigation -->
			<?php
...
if ( ! function_exists( 'fw_theme_post_nav' ) ) : /**
	...
	function fw_theme_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'taxipark' ); ?></h1>

			<div class="nav-links">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link',
					esc_html__( '<span class="meta-nav">Published In</span>%title', 'taxipark' ) );
				else :
					previous_post_link( '%link',
					esc_html__( '<span class="meta-nav">Previous Post</span>%title', 'taxipark' ) );
					next_post_link( '%link', esc_html__( '<span class="meta-nav">Next Post</span>%title', 'taxipark' ) );
				endif;
				?>
				</div>
				<!-- .nav-links -->
...
if ( ! function_exists( 'fw_theme_posted_on' ) ) : /**
	...
	function fw_theme_posted_on() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="featured-post">' . esc_html__( 'Sticky', 'taxipark' ) . '</span>';
		}
